...
function projectInfo(json) {
  ...
  if (keywordCorrect == false) {
    ...
    short_description.textContent =
      json["projects"][pv]["short-description"];
    year.textContent = json["projects"][pv]["year"];
    project_title.style.backgroundImage = "linear-gradient(to top, var(--dark), rgba(0, 0, 0, 0)), url('images/" + json["projects"][pv]["big-cover"] + "')";
    ...
    for (var i = 0 ; i < json["projects"][pv]["author"].length ; i++) {
        if (json["projects"][pv]["author"][i][1] == "") {
            ...
            author.innerHTML += "<a target='blank' class='link' href='" +  json["projects"][pv]["author"][i][1] + "'>" + json["projects"][pv]["author"][i][0] + "</a><br>";
        }
    }
    ...
    if (json["projects"][pv]["awards"].length == 0) {
        ...
        for (var i = 0 ; i < json["projects"][pv]["awards"].length ; i++) {
            awards.innerHTML += "<a target='blank' class='link' href='" +  json["projects"][pv]["awards"][i][1] + "'>" + json["projects"][pv]["awards"][i][0] + "</a><br>";
        }
    }
    ...
    if (json["projects"][pv]["client"].length == 0) {
        ...
        for (var i = 0 ; i < json["projects"][pv]["client"].length ; i++) {
            client.innerHTML += "<a target='blank' class='link' href='" +  json["projects"][pv]["client"][i][1] + "'>" + json["projects"][pv]["client"][i][0] + "</a><br>";
        }
    }
    ...
    for (var i = 0; i < json["projects"][pv]["images"].length; i++) {
        if (json["projects"][pv]["images"][i][1] == "two") {
          ...
          imgDivImg.src =
            "images/" + json["projects"][pv]["images"][i][0];
            img_container.append(imgDivImg);
            imgDivImg.style.backgroundImage =
              "url(images/" +
              json["projects"][pv]["images"][i][0] +
              ")";
